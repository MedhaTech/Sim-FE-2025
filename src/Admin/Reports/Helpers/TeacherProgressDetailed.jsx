...
const TeacherProgressDetailed = () => {
    ...
    var radialChart = {
        ...
        series: [Math.round(totalCount.courseCompleted*100/totalCount.totalReg), Math.round(totalCount.courseINcompleted*100/totalCount.totalReg),  Math.round((totalCount.totalReg -(totalCount.courseCompleted + totalCount.courseINcompleted))*100/totalCount.totalReg)],
        ...
        axios(config)
            .then(function (response) {
                ...
                if (response.status === 200) {
                    ...
                    const postSurveyMap = response.data.data[0].postSurvey.reduce((map, item) => {
                    ...
                    const teamCountMap = response.data.data[0].teamCount.reduce((map, item) => {
                    ...
                    const StudentCourseCmpMap = response.data.data[0].StudentCourseCmp.reduce((map, item) => {
                        map[item.mentor_id] = item.countop;
                        return map;
                    }, {});
                    const StudentCourseINproMap = response.data.data[0].StudentCourseINpro.reduce((map, item) => {
